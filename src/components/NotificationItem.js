...
const NotificationItem = ({ notification, itemClickHandler, readClickHandler, iconClickHandler }) => {

    return (
        <Stack direction='row' alignItems='center' sx={{ width: '100%' }} /*onClick={() => itemClickHandler(notification.id)}*/>
            <IconButton sx={{ p: 0, mr: 2 }} onClick={() => iconClickHandler(notification.sender.id)}>
                <Icon user={notification.sender} />
            </IconButton>
